...
if len(fin)>1:
    ...
    if fin[-3:-1]=='ff':    # FF file
        ...
        try:
            ...
            Eps[0]=float(sini[0]); Eps[1]=float(sini[1]); Eps[2]=float(sini[2])
        except:
#            wx.MessageBox(DATADIR+COD+r'_cmvspar.txt не найден, мВ показаны', 'Info', wx.OK | wx.ICON_INFORMATION)
        ...
        ffd[0].close()
        x=x+15018
        y[:,0]=Eps[0]*y[:,0];  y[:,1]=Eps[1]*y[:,1];  y[:,2]=Eps[2]*y[:,2]  # raw data
...
iscorrected=False
font = { 'fontname':'Times New Roman', 'fontsize':16 }
x1=-99999.9;y1=0.0;x2=0.0;y2=0.0;npp=0;npy=0
ch=''
...
tb.AddTool(ON_CUSTOM,'der batton',wx.Image('C:\\bin\\filesave.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Сохранить файл')
wx.EVT_TOOL(tb,ON_CUSTOM, savecor)
#wx.EvtHandler.Bind(ON_CUSTOM, savecor, tb)
